...
> Screenshots are generated by running `bun screenshots` against a local dev server (see [scripts/screenshots.ts](scripts/screenshots.ts)).
...
| ------- | ------------ |
| `bun dev` | Next.js dev server with HMR |
| `bun run build` | Production build |
| `bun start` | Serve the production build |
| `bun typecheck` | `tsc --noEmit` |
| `bun lint` | Next.js ESLint (flat config) |
| `bun db:generate` | Generate SQL migrations from `db/schema.ts` |
| `bun db:migrate` | Apply generated migrations (safe, idempotent) |
| `bun db:push` | Drizzle Kit push (interactive; prefer `db:migrate`) |
...
6. One-time migration against prod: `vercel env pull .env.production.local && bun db:migrate`.
